...
- **New partition bucket.** Today a confirmed order whose claims have not been fetched is reported in the gap strip as *"had a Klaviyo event but no campaign/flow link"* — it reads as a finding about the email program when it is really "not asked yet" (this misled the owner: it showed $14,749). Split it: confirmed + has a conversion event + **no qualifying claim** + no complete claim state becomes `claimsPending`, rendered as "N orders not checked for email links yet". `noEmailLink` then means what it says, and the partition stays exact.

  **Precedence: `email_linked` is decided before `claimsPending`.** A qualifying claim is positive proof that Klaviyo credited a campaign or flow, and positive evidence outranks coverage status: filing proven email revenue under "not checked yet" would understate email — the exact error this bucket exists to prevent. The state is reachable (a claim written by a graph that died before its replay state reached `complete`), so it is pinned by an integration test rather than left implicit. `claimCoverage` still counts such a conversion as unasked, so the caption stays honest while the money stays in the email bucket.
