...
- Filter the already-loaded creative rows client-side and preserve the selection in the page URL. Clear bulk row selection when the UTM filter changes so hidden creatives cannot remain selected.
- Apply the exact filtered creative IDs to export previews and CSV downloads so exports match the visible table.
