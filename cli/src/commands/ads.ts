...
export function registerAdCommands(program: Command) {
  ...
  ads.command("by-set <adSetId>").action(async (adSetId, command) => {
    await runCommand(command, (client) =>
      client.ad.listByAdSet.query({ adSetId }),
    );
  });

  ads.command("by-creative <adCreativeId>").action(async (adCreativeId, command) => {
    await runCommand(command, (client) =>
      client.ad.listByCreative.query({ adCreativeId }),
    );
  });
          ...
          rows: rows.map((row) => ({
            ...
            conversionRateRanking:
              row.conversionRateRanking !== undefined
                ...
                : undefined,
            dateStart:
              row.dateStart !== undefined ? String(row.dateStart) : undefined,
