...
export function registerLandingPageCommands(program: Command) {
  ...
  landingPages.command("get <id>").action(async (id, command) => {
    await runCommand(command, (client) =>
      client.landingPage.getById.query({ id }),
    );
  });
  ...
  landingPages
    ...
    .action(async (options, command) => {
      await runCommand(command, (client) =>
        client.landingPage.create.mutate(
          compactObject({
            name: options.name,
            url: options.url,
          }),
  ...
  landingPages
    ...
    .action(async (id, options, command) => {
      await runCommand(command, (client) =>
        client.landingPage.update.mutate({
          id,
          ...compactObject({
            name: options.name,
            url: options.url,
  ...
  landingPages.command("duplicate <id>").action(async (id, command) => {
    await runCommand(command, (client) =>
      client.landingPage.duplicate.mutate({ id }),
    );
  });

  landingPages.command("versions <landingPageId>").action(async (landingPageId, command) => {
    await runCommand(command, (client) =>
      client.landingPage.listVersions.query({ landingPageId }),
    );
  });
  ...
  landingPages
    ...
    .action(async (landingPageId, options, command) => {
      await runCommand(command, (client) =>
        client.landingPage.createVersion.mutate({
          landingPageId,
  ...
  landingPages
    ...
    .action(async (id, options, command) => {
      await runCommand(command, (client) =>
        client.landingPage.updateVersion.mutate({
          id,
          ...compactObject({
            pageType: options.pageType,
            heroCopy: options.heroCopy,
  ...
  landingPages.command("duplicate-version <id>").action(async (id, command) => {
    await runCommand(command, (client) =>
      client.landingPage.duplicateVersion.mutate({ id }),
    );
  });
}
