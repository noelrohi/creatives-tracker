...
export function registerAdSetCommands(program) {
    const adSets = program.command("ad-sets").description("Manage ad sets");
    adSets.command("list").action(async (_, command) => {
    ...
    adSets
        ...
        await runCommand(command, (client) => client.adSet.create.mutate(compactObject({
    ...
    adSets
        ...
        await runCommand(command, (client) => client.adSet.update.mutate(compactObject({
            id,
            campaignId: options.campaignId,
            name: options.name,
            metaId: options.metaId,
