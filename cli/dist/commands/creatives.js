...
export function registerCreativeCommands(program) {
    ...
    creatives
        ...
        await runCommand(command, (client) => client.adCreative.update.mutate(compactObject({
            id,
            name: options.name,
        ...
        await runCommand(command, async (client) => {
            ...
            return client.adCreative.bulkImport.mutate({
                ...
                rows: rows.map((row) => ({
                    ...
                    landingPageViews: typeof row.landingPageViews === "number"
                    ...
                    initiateCheckout: typeof row.initiateCheckout === "number"
                        ? row.initiateCheckout
                        : undefined,
                    costPerAddToCart: row.costPerAddToCart !== undefined
                        ? String(row.costPerAddToCart)
                        : undefined,
                    videoViews3s: typeof row.videoViews3s === "number" ? row.videoViews3s : undefined,
                    videoThruplay: typeof row.videoThruplay === "number"
                        ? row.videoThruplay
                        : undefined,
                    videoAvgWatchTime: row.videoAvgWatchTime !== undefined
